...
for n,b in original.items():
    ...
    # Derive hierarchy from runtime, not the stale YAML tree.
    originals = {}
    def index_nodes(v):
        if isinstance(v, dict):
            for k,x in v.items():
                if isinstance(x,dict) and ('Properties' in x or 'Control' in x): originals[k]=x
                index_nodes(x)
        elif isinstance(v,list):
            for x in v: index_nodes(x)
    index_nodes(doc)
    def render(c):
        name=c['Name']
        ...
        children=[child for child in c.get('Children',[]) if child.get('Template',{}).get('Name')!='galleryTemplate']
        if children:node['Children']=[{child['Name']:render(child)} for child in children]
        return node
    if 'Screens' in doc:
        doc={'Screens':{name:render(controls[name]) for name in doc['Screens']}}
    elif 'App' in doc:
        doc={'App':render(controls['App'])}
        doc['App'].pop('Children',None)
...
actual_flow = get('tmrLoadingFlow','OnTimerEnd')
assert flow_before[flow_before.index('    IfError('):] == actual_flow[actual_flow.index('    IfError('):]
